...
TypedefDecl *ASTContext::getObjCSelDecl() const {
  if (!ObjCSelDecl) {
    QualType SelT;
    if (getLangOpts().ObjCRuntime.getKind() == ObjCRuntime::KernelObjC){
      SelT = UnsignedShortTy; // Kernel ObjC Runtime uses uint16_t as SEL
    }else{
      SelT = getPointerType(ObjCBuiltinSelTy);
    }
