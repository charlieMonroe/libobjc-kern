...
#define assert(x) objc_assert(x, "Failure on line %d", __LINE__)
									...
									unsigned int size)
{
	...
	assert(0 != attrs);
	assert(objc_strings_equal(types, attrs));
    ...
    for (unsigned int index=0; index<size; index++) {
        ...
        for (unsigned int attrsIndex=0; attrsIndex<attrsCount; attrsIndex++) {
            if (objc_strings_equal(attrsList[attrsIndex].name, list[index].name)) {
                assert(objc_strings_equal(attrsList[attrsIndex].value, list[index].value));
                found = 1;
            }
        }
        assert(found);
    ...
    for (unsigned int index=0; index<size; index++) {
        ...
        assert(0 != value);
        assert(objc_strings_equal(value, list[index].value));
    }
}
