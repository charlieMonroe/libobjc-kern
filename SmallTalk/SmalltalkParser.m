...
MALLOC_DEFINE(M_SMALLTALK_PARSER_TYPE, "Smalltalk Parser", "Smalltalk parser");
...
{
	return objc_zero_alloc(size, M_SMALLTALK_PARSER_TYPE);
}
...
{
	objc_dealloc(memory, M_SMALLTALK_PARSER_TYPE);
}

- (LKAST*) parse:(NSString*)s
{
	unsigned int sLength = (unsigned int)[s length];
